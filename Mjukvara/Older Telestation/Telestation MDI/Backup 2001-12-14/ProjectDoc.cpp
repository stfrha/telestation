...
{
	CMDIBitmapApp*	myApp = (CMDIBitmapApp*) AfxGetApp();

	m_scriptDoc = (CMDIBitmapDoc*) myApp->OpenDocumentFile(m_script.GetItemPath());
	if (!m_scriptDoc) {
		m_scriptDoc = (CMDIBitmapDoc*) myApp->m_scriptTemplate->OpenDocumentFile(NULL, TRUE);
		if (m_scriptDoc) {
