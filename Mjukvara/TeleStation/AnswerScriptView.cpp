// AnswerScriptView.cpp : implementation of the CAnswerScriptView class
//
...
IMPLEMENT_DYNCREATE(CAnswerScriptView, CScrollView)

BEGIN_MESSAGE_MAP(CAnswerScriptView, CScrollView)
	//{{AFX_MSG_MAP(CAnswerScriptView)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	...
	ON_WM_RBUTTONDOWN()
	ON_COMMAND(ID_SCRIPT_BRANCH, OnScriptBranch)
	ON_COMMAND(ID_SCRIPT_PLAY_MEMO, OnScriptPlayMemo)
	ON_WM_LBUTTONDBLCLK()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CAnswerScriptView construction/destruction

CAnswerScriptView::CAnswerScriptView()
{
...
BOOL CAnswerScriptView::PreCreateWindow(CREATESTRUCT& cs)
{
...
/////////////////////////////////////////////////////////////////////////////
// CAnswerScriptView drawing

void CAnswerScriptView::OnDraw(CDC* pDC)
{
	CAnswerScriptDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
void CAnswerScriptView::OnInitialUpdate()
{
	SetScrollSizes(MM_TEXT, GetDocument()->GetDocSize() );

/*
	if (projDoc->m_script.m_openedByProject) {
		if (projDoc->GetDefaultPlacement(GetDocument(), &placement)) {
			GetParentFrame()->SetWindowPlacement(&placement);
		}
	}
*/
	...
	if (pDoc) {
		if (pDoc->m_projectDoc) {
			pDoc->m_projectDoc->LoadViewPlacement(this->GetParentFrame(), 2);
		}
	}
}

/////////////////////////////////////////////////////////////////////////////
// CAnswerScriptView printing

BOOL CAnswerScriptView::OnPreparePrinting(CPrintInfo* pInfo)
{
	// default preparation
	return DoPreparePrinting(pInfo);
}

void CAnswerScriptView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add extra initialization before printing
}

void CAnswerScriptView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add cleanup after printing
}

/////////////////////////////////////////////////////////////////////////////
// CAnswerScriptView diagnostics

#ifdef _DEBUG
void CAnswerScriptView::AssertValid() const
{
	CScrollView::AssertValid();
}

void CAnswerScriptView::Dump(CDumpContext& dc) const
{
	CScrollView::Dump(dc);
}

CAnswerScriptDoc* CAnswerScriptView::GetDocument() // non-debug version is inline
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CAnswerScriptDoc)));
	return (CAnswerScriptDoc*)m_pDocument;
}
#endif //_DEBUG

/////////////////////////////////////////////////////////////////////////////
// CAnswerScriptView message handlers

void CAnswerScriptView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	CAnswerScriptDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
void CAnswerScriptView::OnLButtonUp(UINT nFlags, CPoint point) 
{
...
void CAnswerScriptView::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CAnswerScriptView::OnObjectAddTransition() 
{
	if (m_transitionActive) {
		m_transitionActive = FALSE;
	} else {
...
void CAnswerScriptView::OnUpdateObjectAddTransition(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_transitionActive);
}

void CAnswerScriptView::OnEditPaste( void ) 
{
	m_transitionActive = FALSE;
...
void CAnswerScriptView::ClearDragList( void )
{
...
void CAnswerScriptView::AddToDragList(CRect rect, CScriptObject* object)
{
	m_dragRectList.AddTail(new CDragRect(rect, object));
}

void CAnswerScriptView::AddToDragList( CPoint startP, BOOL startLock, CPoint endP, BOOL endLock)
{
	m_dragLineList.AddTail(new CDragLine(startP, startLock, endP, endLock));
}

void  CAnswerScriptView::ShowDragList( CPoint point )
{
	CClientDC dc( this );
	OnPrepareDC(&dc, NULL);
...
void CAnswerScriptView::HideDragList( void )
{
	CClientDC dc( this );
	OnPrepareDC(&dc, NULL);
...
CRect CAnswerScriptView::GetDragListTestRect( CPoint point)
{
	POSITION	pos;
	CRect		retRect;
...
void CAnswerScriptView::SelectionToDragList(CPoint point)
{
	ClearDragList();
...
int CAnswerScriptView::PointToPointDistance(CPoint p1, CPoint p2)
{
	return (int) sqrt(pow(p1.x - p2.x, 2) + pow(p1.y - p2.y, 2));
}

void CAnswerScriptView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) 
{
	CAnswerScriptDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
void CAnswerScriptView::OnObjectNew(int objectType)
{
	m_transitionActive = FALSE;
...
void CAnswerScriptView::OnScriptDropPhone() 
{
	OnObjectNew(OT_DROP_PHONE);
}

void CAnswerScriptView::OnScriptLiftPhone() 
{
	OnObjectNew(OT_LIFT_PHONE);
}

void CAnswerScriptView::OnScriptWaitForCall() 
{
	OnObjectNew(OT_WAIT_CALL);
}

void CAnswerScriptView::OnScriptPlay() 
{
	OnObjectNew(OT_PLAY_FILE);
}

void CAnswerScriptView::OnScriptAskQuestion() 
{
	OnObjectNew(OT_ASK_QUESTION);
}

void CAnswerScriptView::OnScriptPromptCode() 
{
	OnObjectNew(OT_PROMPT_CODE);
}

void CAnswerScriptView::OnScriptRecMsg() 
{
	OnObjectNew(OT_REC_MSG);
}

void CAnswerScriptView::OnScriptEraseMsg() 
{
	OnObjectNew(OT_ERASE_ALL);
}

void CAnswerScriptView::OnScriptAnswerOn() 
{
	OnObjectNew(OT_ANSWER_ON);
}

void CAnswerScriptView::OnScriptAnswerOff() 
{
	OnObjectNew(OT_ANSWER_OFF);
}

void CAnswerScriptView::OnScriptPlayAll() 
{
	OnObjectNew(OT_PLAY_ALL);
}

void CAnswerScriptView::OnScriptInsertLabel() 
{
	OnObjectNew(OT_LABEL);
}

void CAnswerScriptView::OnScriptBranch() 
{
	OnObjectNew(OT_BRANCH);
}

void CAnswerScriptView::OnScriptPlayMemo() 
{
	OnObjectNew(OT_PLAY_MEMO);
}

void CAnswerScriptView::OnRButtonDown(UINT nFlags, CPoint point) 
{
...
void CAnswerScriptView::OnRButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_disablePopup) {
		CAnswerScriptDoc* pDoc = GetDocument();
		ASSERT_VALID(pDoc);
...
void CAnswerScriptView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	CAnswerScriptDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	...
	CScriptObject*	tmpObject = pDoc->IsObjectHit(displacedPoint);
	if (tmpObject) {
		pDoc->ClearSelection(&dc);
		pDoc->AddToSelection(tmpObject, &dc);
	}
	pDoc->InvokeObjectProperty();
...
void CAnswerScriptView::OnCustomCancel() 
{
	CClientDC dc( this );
	OnPrepareDC(&dc, NULL);
...
void CAnswerScriptView::OnUpdateCustomCancel(CCmdUI* pCmdUI) 
{
