...
#define	OT_BRANCH			0x0E
#define OT_PLAY_MEMO		0x0F




#define NUM_TRANS_TYPE	20
...
public:
	...
	CString	m_genLabel;
	CString	m_instructionStr;
	CString	m_postGoto;
	BOOL	m_processed;
	BOOL	m_writtenToFile;
	BOOL	m_labelIndependent;
	int		m_numCalledBy;
...
class CAnswerScriptDoc : public CTSDocument
{
...
public:
	...
	void	InvokeObjectProperty( void );
	void	GenerateScript(CFileGenDlg* dlg);
	BOOL	DRC(CFileGenDlg* dlg);
	void	GenerateScriptFile(CFileGenDlg* dlg);
	BOOL	LeadsToStart(CScriptObject* obj, CScriptObject* startObj);
	CScriptObject*	FindDepartureLabel(CString label);
	CString	GenerateGenLabelName(CScriptObject* obj);
	BOOL	DoesGenLabelExist(CString label);
	CString CreateInstruction(CScriptObject* obj, int transType, BOOL* processed);
	void	WriteInstruction(FILE* fp, CScriptObject* obj);
	CScriptObject*	FindDstObject(CTransition* trans);
	CString GetInstruction(CScriptObject* obj);
	CString	GetSwitch(CTransition* trans);
	void	GenResetObjects( void );
